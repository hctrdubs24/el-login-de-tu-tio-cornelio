...
DROP DATABASE IF EXISTS TopicosWebLogin;

CREATE DATABASE
    IF NOT EXISTS TopicosWebLogin CHARACTER SET = 'latin1' COLLATE = 'latin1_spanish_ci';

USE TopicosWebLogin;

CREATE TABLE
    IF NOT EXISTS cNombre(
        CvNombre int(3) Auto_Increment Primary Key not null,
        DsNombre varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cNombre (CvNombre, DsNombre)
values (null, 'Diego'), (null, 'Francisco'), (null, 'Daniela'), (null, 'Luis');

CREATE TABLE
    IF NOT EXISTS cApellid(
        CvApellid int(3) Auto_Increment Primary Key not null,
        DsApellid varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cApellid (CvApellid, DsApellid)
values (null, 'Camacho'), (null, 'Espinosa'), (null, 'Díaz'), (null, 'Hernández'), (null, 'Lara'), (null, 'Cruz'), (null, 'Aguilar'), (null, 'Gomez');

CREATE TABLE
    IF NOT EXISTS cGenero(
        CvGenero int(3) Auto_Increment Primary Key not null,
        DsGenero varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cGenero (CvGenero, DsGenero)
values (null, 'Masculino'), (null, 'Femenino');

CREATE TABLE
    IF NOT EXISTS cCalle(
        CvCalle int(3) Auto_Increment Primary Key not null,
        DsCalle varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cCalle (CvCalle, DsCalle)
values (null, 'Benito Juarez'), (null, 'Reforma'), (null, 'Peñarol'), (null, 'Conde Garay');

CREATE TABLE
    IF NOT EXISTS cColon(
        CvColon int(3) Auto_Increment Primary Key not null,
        DsColon varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cColon (CvColon, DsColon)
values (null, 'Xalpa'), (null, 'Cosmopolita'), (null, 'Santa lucia'), (null, 'La libertad');

CREATE TABLE
    IF NOT EXISTS cMunic(
        CvMunic int(3) Auto_Increment Primary Key not null,
        DsMunic varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cMunic (CvMunic, DsMunic)
values (null, 'San cristobal'), (null, 'Soltepec'), (null, 'Novalto'), (null, 'Cordoba');

CREATE TABLE
    IF NOT EXISTS cEstado(
        CvEstado int(3) Auto_Increment Primary Key not null,
        DsEstado varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cEstado (CvEstado, DsEstado)
values (null, 'Sinaloa'), (null, 'Chiapas'), (null, 'Veracuz'), (null, 'Puebla');

CREATE TABLE
    IF NOT EXISTS cTipoPerso(
        CvTipoPerso int(3) Auto_Increment Primary Key not null,
        DsTipoPerso varchar(20) not null
    ) ENGINE InnoDB;

INSERT INTO
    cTipoPerso (CvTipoPerso, DsTipoPerso)
values (null, 'Empleado'), (null, 'Administrador'), (null, 'Proveedor'), (null, 'Cliente');

CREATE TABLE
    IF NOT EXISTS mDirecc(
        ...
        constraint D_fk_cEstado foreign key (CvEstado) references cEstado(CvEstado) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE InnoDB;
...
values (null, 2, 4, 3, 1, '17', '89238'), (null, 1, 2, 1, 2, '475', '51363'), (null, 4, 3, 2, 4, '78', '30232'), (null, 3, 1, 4, 3, '398', '30083');

CREATE TABLE
    IF NOT EXISTS mPerso(
        ...
        constraint Per_fk_mDirecc foreign key (CvDirecc) references mDirecc(CvDirecc) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE InnoDB;
...
CREATE TABLE
    IF NOT EXISTS mUsuario(
        ...
        FechaFin DATE,
        EdoCta BOOLEAN,
        constraint Usr_fk_cPerso foreign key (CvPerso) references mPerso(CvPerso) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE InnoDB;
